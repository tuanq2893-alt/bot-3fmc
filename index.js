...
function createBot() {
  console.log('>>> [1/5] Đang kết nối trực tiếp đến server 3FMC...');
  ...
  bot.on('spawn', () => {
    console.log('>>> [2/5] hkurxf ĐÃ VÀO LOBBY SUCCESS! <<<');
    ...
    bot.once('windowOpen', async (window) => {
      console.log('>>> [5/5] Đã mở Menu! Đang chọn SMP Tu Tiên...');
    ...
    setTimeout(() => {
      ...
      setInterval(() => {
        ...
        bot.activateItem(); 
        console.log(`[TU LUYỆN] hkurxf đang đổi ô ${currentSlot + 1}`);
      }, 10000);
  ...
  bot.on('error', err => {
    console.log('Lỗi kết nối Render:', err.message);
  });
}
